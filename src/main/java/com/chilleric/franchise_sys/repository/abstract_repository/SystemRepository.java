...
public class SystemRepository<T> extends AbstractRepository {
  ...
  public Optional<List<T>> getListByAttribute(String value, String attribute) {
    ...
    List<T> list = systemFind(query, g()).get();
    if (list.size() > 0) {
  ...
  public void deleteById(String id) {
    try {
      ...
      systemDBTemplate.remove(query, g());

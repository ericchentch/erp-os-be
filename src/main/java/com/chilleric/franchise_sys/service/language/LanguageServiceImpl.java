...
public class LanguageServiceImpl
  extends AbstractService<LanguageRepository>
  ...
  public Optional<ListWrapperResponse<LanguageResponse>> getLanguages(
    ...
    List<Language> languages = repository
      ...
      .get();
    return Optional.of(
      new ListWrapperResponse<LanguageResponse>(
        languages
          .stream()
          .map(
            lang ->
  ...
  public void addNewLanguage(LanguageRequest languageRequest) {
    ...
    repository
      .getEntityByAttribute(languageRequest.getKey().toLowerCase(), "key")
      .ifPresent(
        language -> {
    ...
    Language languageDefault = repository
      .getEntityByAttribute("en", "key")
      .orElseThrow(
        () ->
    ...
    Language language = new Language(
      ...
      languageRequest.getKey().toLowerCase(),
      languageDefault.getDictionary()
    );
  ...
  public void deleteDictionaryKey(String dictKey) {
    repository
      .getListOrEntity(new HashMap<>(), "", 0, 0, "")
      .get()
      .forEach(
        language -> {
    ...
    payload.forEach(
      item -> {
        if (StringUtils.hasText(item.getId())) {
          repository
            .getEntityByAttribute(item.getId(), "_id")
            .ifPresent(
              langUpdate -> {
  ...
  public Optional<LanguageResponse> getLanguageByKey(String key) {
    List<Language> languages = repository
      .getListOrEntity(Map.ofEntries(entry("key", key.toLowerCase())), "", 0, 0, "")
      .get();
    ...
    if (languages.size() == 0) {
      return Optional.of(
        new LanguageResponse(
          ...
          languageDefault.get(0).getLanguage(),
          languageDefault.get(0).getKey(),

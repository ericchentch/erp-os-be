...
public class NavbarServiceImpl extends AbstractService<NavbarRepository> implements NavbarService {
                        ...
                        .map(navbar -> new NavbarResponse(navbar.get_id().toString(),
                                ...
                                navbar.getMainSidebar().stream()
                                        .filter(thisPath -> pathInventory
                                                .findPathById(thisPath.toString()).isPresent())
                                        .map(thisPath -> {
                                            Path path = pathInventory
                                                    .findPathById(thisPath.toString()).get();
                                            return new PathResponse(path.get_id().toString(),
                                                    path.getLabel(), path.getPath(), path.getType(),
                                                    path.getIcon());
                                        }).collect(Collectors.toList()),
                                navbar.getMainSidebar().stream()
                                        .filter(thisPath -> pathInventory
    ...
    public Optional<NavbarResponse> getNavbarDetailById(String id) {
        ...
        return Optional.of(new NavbarResponse(navbar.get_id().toString(), navbar.getName(),
                ...
                navbar.getMainSidebar().stream().filter(
                        thisPath -> pathInventory.findPathById(thisPath.toString()).isPresent())
                        .map(thisPath -> {
                            Path path = pathInventory.findPathById(thisPath.toString()).get();
                            return new PathResponse(path.get_id().toString(), path.getLabel(),

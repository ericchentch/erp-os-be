...
public class ShiftServiceImpl extends AbstractService<ShiftRepository> implements ShiftService {
	...
	public void createShift(ShiftRequest shiftRequest) {
		...
		String startCombine = DateFormat.combineHourAndDate(shiftRequest.getStartDate(),
				shiftRequest.getStartHour());
		String endCombine =
				DateFormat.combineHourAndDate(shiftRequest.getEndDate(), shiftRequest.getEndHour());
		shift.setStartTime(
	...
	public Optional<ListWrapperResponse<ShiftResponse>> getShifts(Map<String, String> allParams,
		...
		List<Shift> shifts =
				repository.getShifts(allParams, keySort, page, pageSize, sortField).get();
	...
	public Optional<ShiftResponse> searchShiftByName(String shiftName) {
		Shift shift = shiftInventory.findShiftByName(shiftName).orElseThrow(
				() -> new ResourceNotFoundException(LanguageMessageKey.SHIFT_NOT_FOUND));
		return Optional.of(new ShiftResponse(shift.get_id().toString(), shift.getShiftName(),
				DateFormat.toDateString(shift.getStartTime(), DateTime.YYYY_MM_DD_HH_MM_SS_HYPHEN),
				DateFormat.toDateString(shift.getEndTime(), DateTime.YYYY_MM_DD_HH_MM_SS_HYPHEN)));
	...
	public void deleteShift(String shiftId) {
		shiftInventory.findShiftById(shiftId).orElseThrow(
				() -> new ResourceNotFoundException(LanguageMessageKey.SHIFT_NOT_FOUND));

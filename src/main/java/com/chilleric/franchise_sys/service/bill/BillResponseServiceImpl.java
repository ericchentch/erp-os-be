...
@Service
public class BillResponseServiceImpl
  ...
  @Override
  public List<DraftDetailResponse> getListDraftDetailResponse(
    ...
    return draftDetailList
      ...
      .map(
        draftDetail -> {
          ...
          if (draftDetail.getType() == TypeObjectBill.ROOMTYPE) {
            objectResponse =
              roomTypeService
                .getRoomTypeById(draftDetail.getId().toString())
                .orElse(null);
          }
      ...
      .filter(e -> e.getObjectResponse() != null)
      ...
      .map(
        timeline ->
          new TimelineResponse(
            timeline.getPaymentMethod(),
            timeline.getAmountOfPayment(),
            timeline.getCreated().toString()
          )
      )
      .collect(Collectors.toList());
  }
      ...
      .map(
        element -> {
          ...
          if (element.getType() == TypeObjectBill.ROOMTYPE) {
            RoomTypeResponse roomTypeResponse = roomTypeService
              .getRoomTypeById(element.getId().toString())
              .orElseThrow(
                () ->
  ...
  public float getPaidTotal(List<Timeline> timelineList) {
    return timelineList
      .stream()
      .map(Timeline::getAmountOfPayment)
      .reduce((float) 0, Float::sum);
  }
}
